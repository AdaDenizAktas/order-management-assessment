...
function patchApiTsconfig() {
  const p = path.join("apps", "api", "tsconfig.json");
  const j = readJson(p);
  j.compilerOptions = j.compilerOptions || {};

  // Revert from NodeNext strictness; keep ESM output, but allow ".js" specifiers in TS.
...
function patchRelativeSpecifiersToJsExt() {
  const root = path.join("apps", "api", "src");
